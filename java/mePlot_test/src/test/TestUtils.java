...
public abstract class TestUtils{
	...
	protected static void checkWithDerivatives(final String function, final String derivative,
	...
	protected final void assertSimplify(final Expression input, final String expected, final boolean doManual){
		...
		final IExpressionIterator step = SimplificationHelper.stepSimplify(input);
	...
	private IExpressionList manualSimplify(final Expression input){
		...
		for(int safety = 0; safety < max && next != null && !current.isIdentical(next); safety++){
			...
			if(safety == 99)
				// ESCA-JAVA0285:
				Log.log(LogLevel.WARNING, "Exceeded 100 iterations in stepSimplify(), last was "
	...
	protected static void checkPrecision(final double val, final double target, final double percent){
		...
		if(percent == 0)
			assertEquals("Too low precision, expected " + Double.toString(target) + " got " + Double.toString(val)
					+ ", so error was " + Double.toString(delta) + " but tolerance was 0", delta, 0, Double.MIN_VALUE);
