...
public class PlotterTest extends TestUtils{
	...
	public void testOde(){
		...
		final RungeKutta rk = new RungeKutta(parseOrFail("{{0,1},{5,1}}x"), 0,
				parseOrFail("tr{3,0}"), parseOrFail("{1,0}"));
		rk.start(0.1);
	...
	public void testXP(){
		...
		checkNormalGraph(dc, "x+1", "C(0)R(0,0,10,10)C(3158080)L(0,0,0,10)L(1,0,1,10)"
				...
				+ "L(8,1,9,0)C(16711680)L(1,1,1,1)C(65280)L(1,2,1,2)C(255)L(1,3,1,3)"
				+ "C(0)F()");
	}

	private static void checkNormalGraph(final DrawController controller,
			final String function, final String result){
		final AbstractGraphics graphics = new StringGraphics();
		...
		assertEquals("Graphic for " + function + " is not the same", result,
				graphics.toString());
	}
}
