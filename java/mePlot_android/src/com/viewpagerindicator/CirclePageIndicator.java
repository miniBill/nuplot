...
public class CirclePageIndicator extends View implements PageIndicator{
	...
	protected void onDraw(final Canvas canvas){
		...
		final int count = getModdedCount();
		...
		float cx = mod(mSnap ? mSnapPage : mCurrentPage) * threeRadius;
	...
	private int mod(final int arg){
		if(mModulus != 0)
			return arg % mModulus;
		return arg;
	}

	private int getModdedCount(){
		if(mModulus != 0)
			return mModulus;
	...
	public boolean onTouchEvent(final MotionEvent event){
		if(event.getAction() == MotionEvent.ACTION_DOWN){
			final int count = getModdedCount();
			final int longSize = mOrientation == HORIZONTAL ? getWidth()
					: getHeight();
			final float halfLongSize = longSize / 2.0f;
			final float halfCircleLongSize = count * 3 * mRadius / 2;
			final float pointerValue = mOrientation == HORIZONTAL ? event
					.getX() : event.getY();
	...
	static class SavedState extends BaseSavedState{
		...
		public static final Parcelable.Creator<SavedState> INDICATOR_CREATOR = new Parcelable.Creator<SavedState>(){
			@Override
