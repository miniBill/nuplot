...
public final class PolynomialMath{
	...
	private static IExpressionList getRoots(final Poly poly, final char var){
		...
		final IExpressionIterator leadIterator = leadDiv.getIterator();
		while(leadIterator.hasNext()){
			final Expression leadCurr = leadIterator.next();
			final IExpressionIterator trailIterator = trailDiv.getIterator();
	...
	private static IExpressionList divisors(final Multiplication expr){
		final IExpressionIterator factors = expr.getFactors();
	...
	private static void gen(final IExpressionIterator iterator, final IExpressionList[] gens){
	...
	public static IExpressionList divisors(final Fraction frac){
		...
		final IExpressionIterator denIter = denDiv.getIterator();
		final IExpressionList toret = new ExpressionList();
		while(denIter.hasNext()){
			final Expression denCurr = denIter.next();
			final IExpressionIterator numIter = numDiv.getIterator();
	...
	/*
	 * public static Expression divide(final Poly num, final Poly den){
	 * Expression toret = Int.ZERO; final ExpressionIterator ncoeff =
	 * num.getCoefficents(); final ExpressionIterator dcoeff =
	 * den.getCoefficents(); if(dcoeff.length() == 2){ final Expression first =
	 * dcoeff.next(); final Expression second = dcoeff.next(); final Expression
	 * root = first.divide(second).opposite().simplify(); final Expression[]
	 * narr = ncoeff.toArray(); Expression last = narr[narr.length - 1]; toret =
	 * narr[narr.length - 1].multiply(new Power(num.getLetter(), narr.length -
	 * 2)); for(int i = 1; i < narr.length - 1; i++){ final Expression expr =
	 * new Power(num.getLetter(), narr.length - 2 - i); final Expression coeff =
	 * narr[narr.length - i - 1].add(last); toret =
	 * toret.add(coeff.multiply(expr)); last = coeff.multiply(root); }
	 * if(!last.add(narr[0]).simplify().isZero()){ throw new
	 * CalcException("Division unsuccessfull. Remainder nonzero"); } return
	 * toret.multiply(second); } return toret; }
	 */
}
