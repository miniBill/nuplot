...
public final class SimplificationHelper{
	...
	public static Expression simplify(final Expression arg){
		...
		for(int safety = 0; safety < max; safety++){
			...
			if(safety == MAX_SIMPLIFY_ITERATIONS - 1){
				...
				Log.log(LogLevel.WARNING, "Exceeded " + MAX_SIMPLIFY_ITERATIONS
						+ " iterations in Simplify(), last was " + nextString);
			}
	...
	public static IExpressionIterator stepSimplify(final Expression arg){
		if(arg.isSimplified())
