...
import meplot.expressions.list.IValueListIterator;
...
public final class Lambda extends AbstractExpression{
	...
	public Expression partialSubstitute(final IValueList valueList){
		...
		final IValueListIterator iterator = valueList.getIterator();
	...
	public Expression add(final Expression other){
		if(other instanceof Lambda){
			...
			if(lother.right.hasLetter(left))
				return new Lambda(lother.left, lother.right.add(right.partialSubstitute(left, new Letter(lother.left))));
