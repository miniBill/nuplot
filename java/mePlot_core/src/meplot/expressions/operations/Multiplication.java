...
public final class Multiplication extends AbstractExpression implements IMultiplication{
	...
	public Multiplication(final IExpressionIterator iterator){
	...
	public Expression expand(){
		...
		final IExpressionIterator iterator = getFactors();
		while(iterator.hasNext()){
			...
			if(curr instanceof Sum){
				...
				final IExpressionIterator add = ((Sum)curr).getAddends();
	...
	public void toString(final StringBuffer buffer){
		...
		while(iterator.hasNext()){
			...
			if(isMinusOne(curr))
				continue;
			if(curr instanceof Letter || curr instanceof IFunction || curr instanceof Power || curr instanceof Matrix){
	...
	public Expression innerSimplify(){
		...
		IExpressionIterator iterator = getFactors();
	...
	private static Expression simplifyFactors(final IExpressionIterator iterator){
	...
	private static Expression stepSimplifyFactors(final IExpressionIterator iterator){
	...
	private static Expression innerMultiply(final Multiplication after, final IExpressionIterator iterator){
	...
	private static Expression finishInner(final Multiplication after, final Expression left, final Expression right,
		...
		final IExpressionIterator iterator = after.getFactors();
	...
	private boolean nCompatible(final Expression arg){
		...
		final IExpressionIterator iterator = factors.getIterator();
		boolean usd = false;
		while(iterator.hasNext()){
			final Expression curr = iterator.next();
			if(curr instanceof INumber)
	...
	private static boolean nCheckContains(final IExpressionList small, final IExpressionList big){
		final IExpressionIterator smallIt = small.getIterator();
		final boolean[] usd = new boolean[big.length()];
		while(smallIt.hasNext()){
			final Expression curr = smallIt.next();
			if(curr instanceof INumber)
				continue;
			final IExpressionIterator bigIt = big.getIterator();
	...
	private static boolean nCheckContainsSearch(final boolean[] usd, final Expression curr,
			final IExpressionIterator bigIt){
	...
	private static INumber getCoefficent(final IExpressionList list){
		...
		final IExpressionIterator iterator = list.getIterator();
		while(iterator.hasNext()){
			final Expression curr = iterator.next();
			if(curr instanceof INumber)
