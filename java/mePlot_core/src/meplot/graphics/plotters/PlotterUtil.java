...
public final class PlotterUtil{
	...
	public static boolean[][] buildBCache(final WeakReference bcacheRef, final int dim1, final int dim2){
		final Object ref = bcacheRef.get();
		final boolean[][] bcache = ref == null ? null : (boolean[][])ref;
		if(bcache == null || bcache.length != dim1 || bcache.length > 0 && bcache[0].length != dim2)
			return new boolean[dim1][dim2];
		return bcache;
	}

	public static float[][] buildFCache(final WeakReference rccacheRef){
		return buildFCache(rccacheRef, width, height);
	}

	public static float[][] buildFCache(final WeakReference rccacheRef, final int dim1, final int dim2){
		final Object ref = rccacheRef.get();
		final float[][] rccache = ref == null ? null : (float[][])ref;
	...
	private static double project(final double val, final double minf, final double maxf, final int mint, final int maxt){
