...
import Glsl exposing (BisectSignature, Stat(..), Statement, Vec3, dot2X, dot2Y, dot3X, dot3Y, dot3Z, float, int)
...
glslFromSolutions suffix sols =
    fun4 boolT ("bisect" ++ suffix) (vec3T "o") (mat3T "d") (floatT "max_distance") (out vec3T "found") <|
        \o d maxDistance found ->
            def vec3T "dMix" (mix331 (array33 d (int 0)) (array33 d (int 1)) (float 0.5)) <|
                \dMix ->
                    def floatT "t" (by11 maxDistance <| float 2) <|
                        \t ->
                            sols { dMix = dMix, o = o, t = t } <|
                                expr (assignOut found (add33 o (by13 t (mix331 (array33 d (int 0)) (array33 d (int 1)) (float 0.5))))) <|
